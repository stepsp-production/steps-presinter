/* لا تستخدم import؛ كل شيء UMD من /vendor أو CDN */
...
function highestAvcIndex(levels){
  let m=-1; (levels||[]).forEach((l,i)=>{ if(isAvc(l)) m=i; });
...
function containsTime(v,t){
  try{
    const b=v.buffered; for(let i=0;i<b.length;i++){ if(t>=b.start(i)&&t<=b.end(i)) return {start:b.start(i),end:b.end(i)}; }
  }catch(e){}
  return null;
}
function nearestBufferedTime(v,t){
  try{
    ...
    for(let i=0;i<b.length;i++){
      ...
      const d=Math.abs(cand-t); if(d<dm){ dm=d; best=cand; }
    }
    return best;
  }catch(e){ return null; }
}
...
function attachHlsWithAvc(video,url){
  ...
  hls.on(Hls.Events.ERROR,(_,err)=>{
    // أخطاء parsing TS من البروكسي ليست قاتلة غالبًا
...
function initMain(){
  const mainSrc = (window.sources && window.sources.main) || '';
  const {wrap,video}=createVideoElement(mainSrc);
...
function getOrCreateCam(id){
  ...
  if(ent) return ent;
  const src = window.sources ? window.sources[id] : '';
  const {wrap,video}=createVideoElement(src);
...
async function setActiveCamSmooth(id){
  if(!window.sources || !window.sources[id] || id===currentCam) return;
...
/* ================== LiveKit: اقتران/نشر ================== */
/* تحميل SDK تلقائيًا من الـCDN إذا لم يوجد ككائن عالمي */
function getLK(){
...
function ensureSDK() {
  ...
  const LK=getLK();
  if(!LK || !LK.Room || !LK.createLocalTracks){
    throw new Error('LiveKit SDK loaded but global object missing expected members');
  }
  return LK;
}
...
pairBtn.addEventListener('click', async ()=>{
  try{
    ...
    alert('تعذّر الاقتران — تأكد من منح إذن الكاميرا/المايك، أو من الشبكة لتحميل LiveKit SDK.');
...
publishBtn.addEventListener('click', async ()=>{
  try{
    ...
    alert('تعذر النشر — إذا استمر، فالمشكلة غالبًا من سيرفر /token أو الشبكة.');
...
/* ===== Debug HLS اختياري (للكونسول) ===== */
(function addHlsDebug(){
  function wireDebug(video){
    const h=video && video.__hls; if(!h) return;
    h.on(Hls.Events.MANIFEST_PARSED,(_,data)=>{ console.log('[HLS] MANIFEST_PARSED levels=', (data?.levels||[]).map(l=>({h:l.height,codecs:l.codecs})));});
