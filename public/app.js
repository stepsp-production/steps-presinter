...
function attachHlsWithAvc(video,url){
  ...
  hls.on(Hls.Events.ERROR,(_,err)=>{
    // معظم أخطاء parsing/frag ليست قاتلة؛ اترك HLS يعيد المحاولة
    if(!err?.fatal){ console.debug('[HLS] non-fatal', err?.details||err); return; }
    if(err.type==='mediaError'){
      try{hls.recoverMediaError();}
      catch(e){ try{hls.destroy();}catch(_){} try{attachHlsWithAvc(video,url);}catch(__){} }
    }else{
...
/* ===== LiveKit: اقتران/نشر (محدّث) ===== */

// نحاول التقاط الـ UMD أيًا كان الاسم على النافذة
function getLK(){
  return (window.livekit || window.LiveKit || window.Livekit || window.LiveKitClient || null);
}

// انتظر حتى تُحمّل مكتبة livekit (لأنها defer) قبل تفعيل الأزرار
function waitForSDK(timeoutMs=8000){
  const t0 = performance.now();
  return new Promise((resolve, reject)=>{
    (function loop(){
      const LK = getLK();
      if (LK && LK.Room && LK.createLocalTracks) return resolve(LK);
      if (performance.now() - t0 > timeoutMs) return reject(new Error('LiveKit SDK not loaded'));
      setTimeout(loop, 100);
    })();
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // اقتران: طلب أذونات + إنشاء المسارات
    ...
    publishBtn.addEventListener('click', async () => {
      try {
        ...
        // يجب أن يُرجع السيرفر { url, token }
        const url = `https://steps-livekit-api.onrender.com/token?room=${encodeURIComponent(roomName)}&identity=${encodeURIComponent(identity)}`;
        ...
        await lkRoom.connect(apiUrl, token);
        ...
        alert('تعذر نشر الصوت/الفيديو — تحقق من توكن/الخادم والاتصال.');
    ...
    console.error(e);
    alert('LiveKit SDK غير مُحمَّل — تأكد من /vendor/livekit-client.umd.min.js.');
    setLKStatus('SDK غير مُحمَّل');
  }
});
