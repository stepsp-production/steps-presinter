...
pairBtn.addEventListener('click', async () => {
  try{
    ...
    const req = {};
    if (audioId) req.audio = { deviceId: { exact: audioId } };
    else req.audio = true;

    if (videoId) {
      req.video = { deviceId: { exact: videoId }, resolution: (LK.VideoPresets && LK.VideoPresets.h720) ? LK.VideoPresets.h720 : undefined };
    } else {
      req.video = false;
    }
