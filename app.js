/* =======================================================================
   HLS إعدادات ومحركات تشغيل (كما في كودك الأصلي مع تحسينات التحمل)
   ======================================================================= */
...
/* =======================================================================
   تهيئة المشغلات وبناء قائمة القنوات
   ======================================================================= */
function initMain(){
...
/* =======================================================================
   مُحمِّل LiveKit المحسّن (بدون import – يعمل مع UMD)
   ======================================================================= */
...
function __pickLK() {
  ...
  return (LK && (LK.Room || LK.RoomConnect) && (LK.createLocalTracks || LK.createLocalAudioTrack || LK.createLocalVideoTrack))
    ? LK
    : null;
}
async function __waitForLK(timeoutMs=2500) {
...
function __requireLiveKitOrAlert() {
  const LK = __pickLK();
  if (!LK) {
    alert('LiveKit SDK غير مُحمّل — تأكد من السماح للـCDN في CSP ووجود /vendor/livekit-client.umd.min.js محليًا.');
    return null;
  }
  return LK;
}
async function ensureLKReadyOrAlert() {
  ...
  if (!ok) { alert('LiveKit SDK غير مُحمّل — تعذّر التحميل من CDN و/أو المسار المحلي.'); return null; }
  return __requireLiveKitOrAlert();
}
...
pairBtn.addEventListener('click', async () => {
  try{
    const LK = await ensureLKReadyOrAlert(); if(!LK) return;
    ...
    try{
      const tracks = await LK.createLocalTracks({
        audio: true,
        video: { facingMode: 'user', resolution: LK.VideoPresets?.h720 || {width:1280,height:720} }
      });
      localTracks = tracks;
      ...
      if (err && (err.name === 'NotFoundError' || err.name === 'OverconstrainedError')) {
        alert('لم يتم العثور على جهاز كاميرا/ميكروفون مناسب. تحقق من توصيل الأجهزة أو اختر جهازًا مختلفًا من متصفحك.');
      } else if (err && err.name === 'NotAllowedError') {
        alert('تم رفض الإذن بالكاميرا/المايك. يرجى السماح من المتصفح ثم إعادة المحاولة.');
      } else {
    ...
    console.error('Pair error (outer):', err);
    alert('تعذر الوصول للكاميرا/المايك.');
    setLKStatus('فشل الاقتران');
  }
});
...
publishBtn.addEventListener('click', async () => {
  ...
  try{
    ...
    const res = await fetch(`https://https://steps-presinter.onrender.com/token?room=${encodeURIComponent(roomName)}&identity=${encodeURIComponent(identity)}`);
    if(!res.ok){
      publishBtn.disabled = false;
    ...
    const url = data && (data.url || data.wss || data.ws) || null;
    const token = data && (data.token || data.jwt) || null;
    ...
    alert('تعذر نشر الصوت/الفيديو — تحقق من صحة URL/Token ومن السماح بالميك/الكاميرا.');
