...
const build = (p) => `${HLS_BASE}${p}`.replace(/(?<!:)\/{2,}/g, '/');
...
const channelMap = [
  ...
  { id: 'cam7', label: 'SNEFLEX',src: sources.cam7 },
];
...
function pickBestAvc(levels,cap=480){
...
async function waitBufferedAt(v,t,minAhead=SHOW_MIN_BUF,timeout=6000){t=capLiveEdge(v,t);const t0=performance.now();return new Promise(res=>{(function loop(){const r=containsTime(v,t);if(r&&r.end-t>=minAhead)return res(true);if(performance.now()-t0>timeout)return res(false);setTimeout(loop,90);})();});}
...
function attachHlsWithAvc(video,url){
  if (window.Hls && Hls.isSupported()){
    ...
    video.__hls=hls; hls.attachMedia(video);
    ...
    hls.on(Hls.Events.MANIFEST_PARSED,(_,data)=>{try{
      ...
      if(s>=0){hls.startLevel=s;hls.currentLevel=s;hls.nextLevel=s} if(mx>=0){hls.autoLevelCapping=mx}
    }catch(e){}});
    hls.on(Hls.Events.ERROR,(_,err)=>{ if(err?.fatal){ if(err.type==='mediaError'){try{hls.recoverMediaError();}catch(e){try{hls.destroy();}catch(_){} try{attachHlsWithAvc(video,url);}catch(__){}}} else {try{hls.destroy();}catch(e){} try{attachHlsWithAvc(video,url);}catch(_){}} } });
  ...
  if (video.canPlayType('application/vnd.apple.mpegURL')) video.src=url; else video.src=url;
  return null;
}
...
function initMain(){const {wrap,video}=createVideoElement(sources.main); mountTo(mainContainer,wrap); wrap.style.opacity='1'; mainPlayer=video;}
function getOrCreateCam(id){
  let ent=playersCache.get(id); if(ent) return ent;
  const {wrap,video}=createVideoElement(sources[id]); mountTo(camContainer,wrap);
...
function buildStreamList(){
  streamList.innerHTML=''; channelMap.filter(ch=>hasUrl(ch.src)).forEach(ch=>{
  ...
  document.addEventListener('keydown',(e)=>{ if(e.target&&['INPUT','TEXTAREA'].includes(e.target.tagName))return;
    const items=[...document.querySelectorAll('.stream-item')]; const n=Number(e.key); if(n>=1&&n<=items.length)items[n-1].click(); });
}
...
[streamPanel,globalControls,document.getElementById('utilityControls'),document.getElementById('chrome')].forEach(el=>{
...
function startTimeTicker(){
  ...
  ticker=setInterval(()=>{ if(isScrubbing) return;
    ...
    if(activePlayer && bufferedAhead(activePlayer)<0.30){const m=getSeekableRange(activePlayer); if(m){const near=Math.max(m.start,m.end-SAFETY_EDGE-STARVED_RESEEK); activePlayer.currentTime=snapIntoBuffer(activePlayer,near);}}
  },250);
}
...
function lk(){ return window.Livekit || window.livekit || window.LiveKit || null; }

pairBtn.addEventListener('click', async ()=>{
  try{
    const L = lk(); if(!L){ alert('LiveKit SDK غير محمّل (CDN)'); return; }
    // هذا يستدعي getUserMedia ويطلب السماح
    localTracks = await L.createLocalTracks({ audio:true, video:true });
    publishBtn.disabled=false; stopBtn.disabled=false;
    setLKStatus('مقترن (جاهز للنشر)');
    alert('تم منح سماح للكاميرا والمايك.');
  }catch(e){
    console.error(e);
    alert('تعذّر الوصول للكاميرا/المايك. امنح الإذن من المتصفح.');
  }
});

publishBtn.addEventListener('click', async ()=>{
  try{
    const L = lk(); if(!L){ alert('LiveKit SDK غير محمّل'); return; }
    const roomName = roomSel.value || 'room-1';
    const identity = (displayName.value||'guest').trim();

    // غيّر للسطر الكامل إذا كانت صفحتك ثابتة ولا يوجد proxy:
    const tokenURL = 'https://steps-livekit-api.onrender.com/token'
                   + `?room=${encodeURIComponent(roomName)}`
                   + `&identity=${encodeURIComponent(identity)}`;

    const resp = await fetch(tokenURL);
    if(!resp.ok) throw new Error('Token API HTTP '+resp.status);
    ...
    setLKStatus('متصل'); alert('تم النشر!');
  }catch(e){
    console.error('تعذّر النشر:', e);
    alert('تعذّر النشر: تأكد من سيرفر التوكن والاتصال.');
  }
});

stopBtn.addEventListener('click', async ()=>{
  try{ if(room){ await room.disconnect(); room=null; } }catch(e){}
  try{ localTracks.forEach(t=>t.stop && t.stop()); localTracks=[]; }catch(e){}
  publishBtn.disabled=true; stopBtn.disabled=true; setLKStatus('غير متصل');
});
 ...
 **********************/
(function debugHls(){
  const wire=(v)=>{const h=v&&v.__hls; if(!h) return;
    h.on(Hls.Events.ERROR,(_,err)=>{ if(err?.fatal) console.error('[HLS]',err); /* غير فتّال: اسكت */ });
  };
