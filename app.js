/* app.js — ثابت، بلا import. يحافظ على مزاياك ويضيف مُحمِّل LiveKit موثوق. */
(() => {
  "use strict";

  /*************  A) مُحمِّل LiveKit موثوق (CDN ثم /vendor)  *************/
  let __lkPromise = null;
  ...
  function hasLiveKit() {
    // تغطية كل الأسماء الشائعة في UMD
    const LK = (window.livekitClient || window.LiveKitClient || window.LiveKit || window.livekit);
    return LK && LK.Room && LK.createLocalTracks ? LK : null;
  }

  async function loadLiveKit() {
    // تحاشي التحميل المكرر
    if (__lkPromise) return __lkPromise;
    __lkPromise = (async () => {
      // إن كانت جاهزة لا نفعل شيئًا
      if (hasLiveKit()) return true;

      // 1) CDN أولاً
      try {
        await createScript('https://cdn.jsdelivr.net/npm/livekit-client@2/dist/livekit-client.umd.min.js');
      } catch (_) {}

      if (hasLiveKit()) return true;

      // 2) النسخة المحلية /vendor (مع كسر كاش)
      try {
        const bust = Date.now().toString(36);
        await createScript(`/vendor/livekit-client.umd.min.js?v=${bust}`);
      } catch (_) {}
  ...
  function requireLiveKitOrAlert() {
    const LK = hasLiveKit();
    if (!LK) {
      alert('LiveKit SDK غير مُحمّل — تأكد من (vendor/livekit-client.umd.min.js) أو السماح للـCDN في الـCSP.');
      return null;
    }
    return LK;
  }

  /*************  B) إعدادات HLS + مزاياك (محتفظ بها)  *************/
  ...
  function attachHlsWithAvc(video,srcUrl){
    ...
    hls.on(Hls.Events.ERROR,(_,err)=>{
      if(!err?.fatal) return; // تحذيرات TS من البروكسي غير قاتلة
  ...
  /*************  D) LiveKit (غرف) — اقتران/نشر بعد ضمان التحميل  *************/
  ...
  async function ensureLKReadyOrAlert() {
    const ok = await loadLiveKit();
    if (!ok) {
      alert('LiveKit SDK غير مُحمّل — تأكد من /vendor أو السماح بالـCDN في الـCSP.');
      return null;
    }
    return requireLiveKitOrAlert();
  }

  pairBtn.addEventListener('click', async () => {
    try {
      const LK = await ensureLKReadyOrAlert();
      if (!LK) return;
      ...
      // iOS قد يحتاج gesture؛ زرّك يحقق ذلك.
      const devices = await navigator.mediaDevices.enumerateDevices();
      const hasCam = devices.some(d => d.kind === 'videoinput');
      const hasMic = devices.some(d => d.kind === 'audioinput');
      if (!hasCam && !hasMic) {
        alert('لا توجد كاميرا/مايك متوفّرة على هذا الجهاز.');
        setLKStatus('لا توجد أجهزة');
        return;
      }
      ...
      // أخطاء شائعة: NotFoundError, NotAllowedError
      if (String(err && err.name).includes('NotFound')) {
        alert('تعذر الوصول للكاميرا/المايك — لم يتم العثور على جهاز. وصّل جهازًا ثم أعد المحاولة.');
      } else if (String(err && err.name).includes('NotAllowed')) {
        alert('تم رفض الإذن — افتح إعدادات المتصفح ومنح إذن الكاميرا/المايك ثم أعد المحاولة.');
      } else {
  ...
  publishBtn.addEventListener('click', async () => {
    try {
      ...
      if(!res.ok){
        publishBtn.disabled = false;
        alert('فشل طلب التوكن (تحقق من السيرفر/الراوت/CORS).');
        setLKStatus('فشل جلب التوكن');
        return;
      }
      const json = await res.json();
      const urlLK = json && json.url;
      ...
      await lkRoom.connect(urlLK, token);
      ...
      alert('تعذر نشر الصوت/الفيديو — تأكد من الشبكة وصحة التوكن.');
